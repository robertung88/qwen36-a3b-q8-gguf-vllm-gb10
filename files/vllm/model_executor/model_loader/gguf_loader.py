...
class GGUFModelLoader(BaseModelLoader):
    ...
    def _get_gguf_weights_map(self, model_config: ModelConfig):
        ...
        # Qwen3.5/Qwen3.6 multimodal wrappers use top-level model_type
        # qwen3_5_moe, but the GGUF architecture maps the text sub-config.
        if (
        ...
        if model_type in ("qwen3_5_text", "qwen3_5_moe_text"):
            ...
            for idx in range(text_config.num_hidden_layers):
                # llama.cpp/Unsloth Qwen3.5 GGUF stores these bare tensors as
                # `ssm_a` and `ssm_dt.bias` (not `.weight`). Keep the legacy
                # aliases too for compatibility with future/other converters.
                gguf_to_hf_name_map[f"blk.{idx}.ssm_a"] = (
                    f"{layer_prefix}.{idx}.linear_attn.A_log"
                )
            ...
            if is_multimodal:
                # Qwen3.5 VL mmproj GGUF stores the projector/merger tensors
                # with legacy `mm.*` / `v.post_ln.*` names.
                gguf_to_hf_name_map["v.post_ln.weight"] = (
                    "model.visual.merger.norm.weight"
                )
                gguf_to_hf_name_map["v.post_ln.bias"] = (
        ...
        if model_type == "qwen3_5_mtp":
            # Qwen3.5/Qwen3.6 GGUF stores the in-model next-token/MTP layer
            # after the base decoder layers as blk.<num_hidden_layers> plus
            # blk.<idx>.nextn.* tensors. vLLM's MTP draft model expects the
            # tensors under mtp.* (then qwen3_5_mtp.py remaps mtp. -> model.).
            base_model_type = getattr(text_config, "model_type", None)
            is_moe = base_model_type == "qwen3_5_moe_text" or hasattr(
                text_config, "num_experts"
            )
            base_layers = getattr(text_config, "num_hidden_layers", 0)
            mtp_layers = getattr(text_config, "mtp_num_hidden_layers", 1) or 1

            gguf_to_hf_name_map["token_embd.weight"] = "model.embed_tokens.weight"
            gguf_to_hf_name_map["output.weight"] = "lm_head.weight"

            for mtp_idx in range(mtp_layers):
                gguf_idx = base_layers + mtp_idx
                prefix = f"mtp.layers.{mtp_idx}"
                gguf_to_hf_name_map[f"blk.{gguf_idx}.attn_norm.weight"] = (
                    f"{prefix}.input_layernorm.weight"
                )
                gguf_to_hf_name_map[f"blk.{gguf_idx}.post_attention_norm.weight"] = (
                    f"{prefix}.post_attention_layernorm.weight"
                )
                gguf_to_hf_name_map[f"blk.{gguf_idx}.attn_q.weight"] = (
                    f"{prefix}.self_attn.q_proj.weight"
                )
                gguf_to_hf_name_map[f"blk.{gguf_idx}.attn_k.weight"] = (
                    f"{prefix}.self_attn.k_proj.weight"
                )
                gguf_to_hf_name_map[f"blk.{gguf_idx}.attn_v.weight"] = (
                    f"{prefix}.self_attn.v_proj.weight"
                )
                gguf_to_hf_name_map[f"blk.{gguf_idx}.attn_output.weight"] = (
                    f"{prefix}.self_attn.o_proj.weight"
                )
                gguf_to_hf_name_map[f"blk.{gguf_idx}.attn_q_norm.weight"] = (
                    f"{prefix}.self_attn.q_norm.weight"
                )
                gguf_to_hf_name_map[f"blk.{gguf_idx}.attn_k_norm.weight"] = (
                    f"{prefix}.self_attn.k_norm.weight"
                )
                gguf_to_hf_name_map[f"blk.{gguf_idx}.nextn.eh_proj.weight"] = (
                    "mtp.fc.weight"
                )
                gguf_to_hf_name_map[f"blk.{gguf_idx}.nextn.enorm.weight"] = (
                    "mtp.pre_fc_norm_embedding.weight"
                )
                gguf_to_hf_name_map[f"blk.{gguf_idx}.nextn.hnorm.weight"] = (
                    "mtp.pre_fc_norm_hidden.weight"
                )
                gguf_to_hf_name_map[f"blk.{gguf_idx}.nextn.shared_head_norm.weight"] = (
                    "mtp.norm.weight"
                )

                if is_moe:
                    gguf_to_hf_name_map[f"blk.{gguf_idx}.ffn_gate_inp.weight"] = (
                        f"{prefix}.mlp.gate.weight"
                    )
                    gguf_to_hf_name_map[f"blk.{gguf_idx}.ffn_gate_inp_shexp.weight"] = (
                        f"{prefix}.mlp.shared_expert_gate.weight"
                    )
                    gguf_to_hf_name_map[f"blk.{gguf_idx}.ffn_down_shexp.weight"] = (
                        f"{prefix}.mlp.shared_expert.down_proj.weight"
                    )
                    gguf_to_hf_name_map[f"blk.{gguf_idx}.ffn_gate_shexp.weight"] = (
                        f"{prefix}.mlp.shared_expert.gate_proj.weight"
                    )
                    gguf_to_hf_name_map[f"blk.{gguf_idx}.ffn_up_shexp.weight"] = (
                        f"{prefix}.mlp.shared_expert.up_proj.weight"
                    )
                    gguf_to_hf_name_map[f"blk.{gguf_idx}.ffn_down_exps.weight"] = (
                        f"{prefix}.mlp.experts.0.down_proj.weight"
                    )
                    gguf_to_hf_name_map[f"blk.{gguf_idx}.ffn_gate_exps.weight"] = (
                        f"{prefix}.mlp.experts.0.gate_proj.weight"
                    )
                    gguf_to_hf_name_map[f"blk.{gguf_idx}.ffn_up_exps.weight"] = (
                        f"{prefix}.mlp.experts.0.up_proj.weight"
                    )
                else:
                    gguf_to_hf_name_map[f"blk.{gguf_idx}.ffn_down.weight"] = (
                        f"{prefix}.mlp.down_proj.weight"
                    )
                    gguf_to_hf_name_map[f"blk.{gguf_idx}.ffn_gate.weight"] = (
                        f"{prefix}.mlp.gate_proj.weight"
                    )
                    gguf_to_hf_name_map[f"blk.{gguf_idx}.ffn_up.weight"] = (
        ...
        if is_multimodal:
            ...
            vision_num_layers = getattr(
                ...
                "num_hidden_layers",
                getattr(config.vision_config, "depth"),
            )
    ...
    def _get_gguf_weight_type(
        ...
        is_multimodal = (
            ...
            and getattr(model_config.hf_config, "model_type", None) != "qwen3_5_mtp"
        )
        if is_multimodal:
    ...
    def load_model(
        ...
        with set_default_torch_dtype(model_config.dtype):
            with target_device:
                # get_model() may pass an explicit model_config that differs from
                # vllm_config.model_config (notably GGUF MTP draft loading). The
                # architecture initializer reads vllm_config.model_config, so use
                # a shallow config replacement for initialization.
                init_vllm_config = (
                    replace(vllm_config, model_config=model_config)
                    if vllm_config.model_config is not model_config
                    else vllm_config
                )
                model = initialize_model(vllm_config=init_vllm_config, prefix=prefix)
