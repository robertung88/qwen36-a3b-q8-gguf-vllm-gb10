...
def _is_gguf_moe_expert_weight(name: str) -> bool:
    # Qwen3.5/Qwen3.6 MoE GGUF can mix Q8_0 and raw BF16 expert tensors.
    # FusedMoE under GGUF registers only *_qweight parameters, even for raw
    # float expert tensors, so force routed expert weights through the qweight
    # path rather than yielding plain *_weight names.
    return (
        ".mlp.experts." in name
...
def _qwen35_even_odd_inverse_permutation(num_heads: int) -> np.ndarray:
    # llama.cpp/Unsloth Qwen3.5 GGUF stores value-head scalars/rows as
    # [even heads..., odd heads...]. HF/vLLM expects natural head order.
    gguf_to_hf = np.concatenate(
        [np.arange(0, num_heads, 2), np.arange(1, num_heads, 2)]
    )
    inverse = np.empty_like(gguf_to_hf)
    inverse[gguf_to_hf] = np.arange(num_heads)
    return inverse


def _qwen35_reorder_vector_even_odd(weight: np.ndarray) -> np.ndarray:
    if weight.ndim != 1 or weight.shape[0] % 2 != 0:
        return weight
    inverse = _qwen35_even_odd_inverse_permutation(weight.shape[0])
    return weight[inverse].copy()


def _qwen35_reorder_value_head_rows(
    weight: np.ndarray,
    *,
    start: int,
    rows: int,
    ...
    if rows <= 0 or rows % head_dim != 0:
        return weight
    num_heads = rows // head_dim
    if num_heads % 2 != 0 or start + rows > weight.shape[0]:
        return weight
    inverse = _qwen35_even_odd_inverse_permutation(num_heads)
    out = weight.copy()
    section = out[start : start + rows]
    out[start : start + rows] = section.reshape(
        num_heads, head_dim, *section.shape[1:]
    )[inverse].reshape(section.shape)
    return out


def _qwen35_reorder_value_head_columns(
    weight: np.ndarray,
    weight_type: Any,
    *,
    head_dim: int = 128,
    ...
    if weight_type.name in ("F32", "F16", "BF16"):
        ...
        if cols % head_dim != 0:
            return weight
        num_heads = cols // head_dim
        if num_heads % 2 != 0:
            return weight
        inverse = _qwen35_even_odd_inverse_permutation(num_heads)
        return weight.reshape(weight.shape[0], num_heads, head_dim)[:, inverse, :].reshape(
            weight.shape
        ).copy()
    ...
    if head_dim % block_size != 0:
        return weight
    encoded_head = head_dim // block_size * type_size
    if weight.shape[1] % encoded_head != 0:
        return weight
    num_heads = weight.shape[1] // encoded_head
    if num_heads % 2 != 0:
        return weight
    inverse = _qwen35_even_odd_inverse_permutation(num_heads)
    return weight.reshape(weight.shape[0], num_heads, encoded_head)[:, inverse, :].reshape(
        weight.shape
    ).copy()


def _maybe_fix_qwen35_gdn_gguf_tensor(
    tensor_name: str,
    hf_name: str,
    weight: np.ndarray,
    weight_type: Any,
) -> np.ndarray:
    # Qwen3.5/Qwen3.6 uses Gemma-style RMSNorm in the HF/vLLM stack:
    # the learned parameter is an offset and forward applies (1 + weight).
    # llama.cpp GGUF stores the already-offset scale, so subtract 1 when
    # loading those RMSNorm tensors. Do not apply this to GDN ssm_norm.
    if tensor_name == "output_norm.weight" or tensor_name.endswith(
        (
            ".attn_norm.weight",
        ...
        return (weight.astype(np.float32) - 1.0).astype(np.float32)

    # Qwen3.5/Qwen3.6 Gated DeltaNet tensors in llama.cpp GGUF use
    # llama.cpp internal value-head order. vLLM expects HF order. Also,
    # GGUF `ssm_a` stores A = -exp(A_log), whereas vLLM expects A_log.
    if ".linear_attn." not in hf_name:
        return weight

    if tensor_name.endswith(".ssm_a") or tensor_name.endswith(".ssm_a.weight"):
        fixed = weight
        if fixed.size and np.all(fixed <= 0):
            fixed = np.log(np.maximum(-fixed, np.finfo(np.float32).tiny)).astype(
                np.float32
            )
        return _qwen35_reorder_vector_even_odd(fixed)

    if tensor_name.endswith(".ssm_dt.bias") or tensor_name.endswith(
        ".ssm_dt.weight"
    ):
        return _qwen35_reorder_vector_even_odd(weight)

    if tensor_name.endswith((".ssm_alpha.weight", ".ssm_beta.weight")):
        if weight.ndim == 2 and weight.shape[0] % 2 == 0:
            inverse = _qwen35_even_odd_inverse_permutation(weight.shape[0])
            return weight[inverse].copy()
        return weight
    ...
    if tensor_name.endswith(".ssm_conv1d.weight"):
        rows = weight.shape[0]
        return _qwen35_reorder_value_head_rows(weight, start=rows // 2, rows=rows // 2)
    ...
    if tensor_name.endswith(".attn_gate.weight"):
        return _qwen35_reorder_value_head_rows(weight, start=0, rows=weight.shape[0])

    if tensor_name.endswith(".ssm_out.weight"):
        return _qwen35_reorder_value_head_columns(weight, weight_type)
...
def gguf_quant_weights_iterator(
    ...
    for tensor in reader.tensors:
        if tensor.name in gguf_to_hf_name_map:
            ...
            force_qweight = (
                weight_type.name not in ("F32", "BF16", "F16")
                or _is_gguf_moe_expert_weight(name)
            )
            if force_qweight:
    ...
    for tensor in reader.tensors:
        if tensor.name in gguf_to_hf_name_map:
            ...
            weight = _maybe_fix_qwen35_gdn_gguf_tensor(
                tensor.name, name, weight, weight_type
            )
...
def default_weight_loader(param: torch.Tensor, loaded_weight: torch.Tensor) -> None:
    ...
    try:
        if param.numel() == 1 and loaded_weight.numel() == 1:
            ...
            # GB10/Qwen3.5 GGUF patch: Qwen3.5 VL mmproj GGUF stores
            # image patch-embed conv weights as 4D [out, in, h, w], while
            # the HF/vLLM module is Conv3d with a singleton temporal patch
            # dimension [out, in, 1, h, w] for text/image-only tests.
            if (
                param.dim() == 5
                and loaded_weight.dim() == 4
                and param.size(2) == 1
                and param.size(0) == loaded_weight.size(0)
                and param.size(1) == loaded_weight.size(1)
                and param.size(3) == loaded_weight.size(2)
                and param.size(4) == loaded_weight.size(3)
            ):
                loaded_weight = loaded_weight.unsqueeze(2)
