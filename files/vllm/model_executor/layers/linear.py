...
class ReplicatedLinear(LinearBase):
    ...
    def weight_loader(self, param: Parameter, loaded_weight: torch.Tensor):
        ...
        # GB10/Qwen35 GGUF patch: some single-output replicated GGUF
        # tensors arrive as [hidden] while vLLM has already materialized the
        # target parameter as [1, hidden]. The upstream PR handles the
        # UninitializedParameter case above; this covers already-materialized
        # parameters.
        if (
            is_gguf_weight
            and loaded_weight.dim() == 1
        ...
        # Final single-output shape compatibility guard for Qwen35 GGUF.
        # Some tensors are not tagged with is_gguf_weight by the time they hit
        # this generic loader, but the shape relationship is unambiguous.
        if (
            loaded_weight.dim() == 1
            and param.dim() == 2
            and param.size(0) == 1
