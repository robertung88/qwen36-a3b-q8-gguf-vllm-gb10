...
class Qwen3_5ForCausalLMBase(
    ...
    def get_mrope_input_positions(
        ...
        # Text-only Qwen3.5/Qwen3.6 configs still carry M-RoPE parameters.
        # For pure text prompts, T/H/W positions are identical monotonically
        # increasing token positions and there is no multimodal position delta.
        if mm_features:
            raise NotImplementedError(
                "Text-only Qwen3.5 CausalLM M-RoPE helper does not support "
                "multimodal features. Use the conditional-generation model "
                "path for vision once GGUF support exists."
            )
        pos = torch.arange(len(input_tokens), dtype=torch.long)
    ...
    # Qwen3.5 text-only hybrid cache helpers. The multimodal wrapper already
    # exposes these, but GGUF text-only resolves to Qwen3_5MoeForCausalLM.
    @classmethod
